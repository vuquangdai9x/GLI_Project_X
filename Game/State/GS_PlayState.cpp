...
void GS_PlayState::Update(float deltaTime)
{
	...
	if (!dynamic_cast<GS_PlayState*>(Singleton<GameStateManager>::GetInstance()->getTop())) return;
	...
	if (playerPos.y > Singleton<SceneManager2D>::GetInstance()->getHeightWin()) {
		int x = Singleton<SceneManager2D>::GetInstance()->getHeightWin();
		if (this->m_PassTime == 0) this->m_PassTime = GetTickCount();
		Vector3 pos;
		pos.x = -30 + rand() % (30 + 1 - (-30));
		pos.y = (playerPos.y-40) + rand() % ( 40 + 1  -(- 40));
		pos.z = -1;
