...
void AnimationController::AddAnimState(int iAnimId, int iTextureId, int iSplitX, int iSplitY, float cycleTime)
{
	...
	if (texture == NULL) {
		printf("[ERR] AnimationController: Can not add Animation. Texture is NULL\n");
		return;
	}
	if (iSplitX <= 0 || iSplitY <= 0) {
		printf("[ERR] AnimationController: Split number is less or equal zero\n");
		return;
	}
	if (cycleTime <=0 ) {
		printf("[ERR] AnimationController: Cycle time is less or equal zero\n");
		return;
	}
	for (int i = 0;i < m_listAnimState.size();i++) {
		if (iAnimId == m_listAnimState[i].id) {
			printf("[Wrn] AnimationController: AnimId existed. This can cause trouble in the future\n");
			break;
		}
	}
...
{
	if (iNumOfTexture <= 0) {
		printf("[ERR] AnimationController: Number of texture is zero\n");
		return;
	}
		...
		if (m_iLoopCounter < 0) { // infinite loop
			...
			if (m_iLoopCounter == 0) { 
				...
				if (m_defaultAnimState == NULL) {
					printf("[Wrn] AnimationController: Can get default animation\n");
				}
				else {
